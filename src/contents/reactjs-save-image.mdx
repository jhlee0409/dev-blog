...
summary: "이미지를 어디에 저장할까"
tags: "react, saveImage"
---

## 서론

`React`나 `Next.js`를 사용하다 보면 `이미지`를 어디에 저장해야할지 고민할 때가다. 크게 이미지를 사용하는 방식은 3가지가 있다.
...
- 세 번째 방법은 `AWS S3`와 같은 외부 스토어에 저장 후 불러와 쓰는 방법이다.
